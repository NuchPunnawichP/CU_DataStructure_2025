...
class map_bst
{
  ...
  public:
    ...
    int checkBalance(node* n, int& maxImbalance, KeyT& ansKey)
    {
        if (n == NULL)
        {
            return -1;
        ...
        int hl = checkBalance(n->left, maxImbalance, ansKey);
        int hr = checkBalance(n->right, maxImbalance, ansKey);
        int h = 1 + (hl > hr ? hl : hr);
    ...
    KeyT getValueOfMostImbalanceNode() {
        ...
        if (mRoot != NULL)
        {
            ans = mRoot->data.first;
            checkBalance(mRoot, maxImbalance, ans);
        }
